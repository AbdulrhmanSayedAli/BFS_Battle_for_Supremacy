...
from bfs_battle_for_supremacy.config import IMAGES_PATH, WIDTH,BOARD_SIZE_WIDTH
    ...
    def update_components(self):
        for i in range(10):
            for j in range(10):
                ...
                self.board.components[i*BOARD_SIZE_WIDTH+j].selected = False
                if data.is_empty:
                    self.board.components[i*BOARD_SIZE_WIDTH+j].image_path = None
                elif isinstance(data.content,Player):
                    if data.content.name=="Player 1":
                        self.board.components[i*BOARD_SIZE_WIDTH+j].image_path=os.path.join(IMAGES_PATH, "player1.webp")
                    else:
                        self.board.components[i*BOARD_SIZE_WIDTH+j].image_path=os.path.join(IMAGES_PATH, "player2.webp")
