...
class PlayerManager:
    ...
    @staticmethod
    def toggle_turn():
        PlayerManager.current_player_index = 1 - PlayerManager.current_player_index

    @staticmethod
    def request_card():
        current_player = PlayerManager.players[PlayerManager.current_player_index]
        enemy_player = PlayerManager.players[1 - PlayerManager.current_player_index]

        card = CardsManager.provide_card(current_player)
        if card and CardsManager.activate_card(card, current_player, enemy_player):
            return card
        return None
    ...
    def place_card_on_map(card, target_square):
        ...
        #    if ResourcesManager.can_afford_card(PlayerManager.players[PlayerManager.current_player_index].resources, card.consumes.get("instant")):
